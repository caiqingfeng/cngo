...
extern char APP_ID[];	// Appid
extern char AUTH_CODE[];	// auth_code
...
void CTraderSpi::ReqAuthenticate()
{
	...
	strcpy(auth.AppID, APP_ID);
	strcpy(auth.AuthCode, AUTH_CODE);
	int iResult = pTradeApi->ReqAuthenticate(&auth, ++iRequestID);
	cerr << "--->>> 发送APP Auth请求: " << ((iResult == 0) ? "成功" : "失败") << endl;
}
		...
		CThostFtdcRspInfoField *pRspInfo, int nRequestID, bool bIsLast)
{
	...
	if (bIsLast && !IsErrorRspInfo(pRspInfo))
	{
		...
		///获取当前交易日
		cerr << "--->>> 获取当前交易日 = " << pTradeApi->GetTradingDay() << endl;
		///投资者结算结果确认
...
//		ReqQryTradingAccount();
        cerr << "--->>> " << pInstrument->PriceTick << endl;
	}
}
...
//	TThostFtdcUserIDType	UserID;
	...
	lOrderTime=time(NULL);
	int iResult = pTradeApi->ReqOrderAction(&req, ++iRequestID);
...
bool CTraderSpi::IsErrorRspInfo(CThostFtdcRspInfoField *pRspInfo)
{
	...
	if (bResult) {
	    ...
	    ofstream myfile;
        myfile.open ("/tmp/ctplog");
    	myfile << "--->>> ErrorID=" << pRspInfo->ErrorID << ", ErrorMsg=" << pRspInfo->ErrorMsg << endl;
        myfile.close();
	}
	return bResult;
}
